...
- **`webui/routes.py`** has three groups: page routes (`/`, `/jobs/<id>`), HTMX fragment routes (`/htmx/models`, `/htmx/effort`, `/htmx/jobs/<id>/status` — polled every 2 s by the job page), and the API export (`/api/jobs/<id>/export.json`).
