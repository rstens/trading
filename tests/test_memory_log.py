...
These tests exercise the **markdown** code path specifically. After
Phase 3 of the persistence rollout, `TradingMemoryLog` delegates to
Postgres when `TRADINGAGENTS_DATABASE_URL` is configured — which would
make every `store_decision` here bypass the markdown file entirely.
The autouse fixture below forces DB off for this module so we always
hit the legacy markdown branch.
"""
...
def _force_markdown_path(monkeypatch):
    """Ensure every test in this module sees DB-off, so TradingMemoryLog
    routes through the markdown branch."""
