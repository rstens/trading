...
def test_provider_effort_overrides(monkeypatch):
    """Reasoning/thinking effort env vars overlay onto the None defaults.

    The matching config keys default to None, so _coerce returns the raw
    string verbatim — the client validates the actual value, not the
    overlay layer.
    """
    dc = _reload_with_env(
        monkeypatch,
...
def test_provider_effort_defaults_none(monkeypatch):
    """With no env set, the effort keys stay None (so clients use their own defaults)."""
    dc = _reload_with_env(monkeypatch)
